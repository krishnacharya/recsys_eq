...
with tqdm(total = tot) as pbar:
    for d in dimension:
        for seed in seeds:
            ...
            nue = generate_sorted_synthetic(dimension = d, num_users = n_users)
            ...
            for nprod in n_prodarr:
                ...
                if converged:
                    dir_prods, prod_utils, user_utils = get_all_engagement_utilities(last_profile, nue, prob_type='softmax')
                    di.update({
                ...
                res.append(di)
df = pd.DataFrame(res)
df.to_pickle('../saved_frames/sorted_synthetic_grid_softmax_statistics.pkl')
