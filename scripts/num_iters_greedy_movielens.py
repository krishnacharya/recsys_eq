...
# TOY search space, comment out part before seed = 17
...
with tqdm(total = tot_loop) as pbar:
    for d in dimensions:
        ...
        for n_prod in num_prod_array:
            PEng = ProducersEngagementGame(num_producers=n_prod, users=users, prob = 'softmax', temp = temp)
            BR_iters = []
            for r in range(runs):
...
df_detailed.to_csv('../csv_results/br_dynamics/greedy_detailed_40runs_movielens.csv', encoding='utf-8', index=False)
df_aggr = pd.DataFrame(res_aggregate)
df_aggr.to_csv('../csv_results/br_dynamics/greedy_aggr_40runs_movielens.csv', encoding='utf-8', index=False)