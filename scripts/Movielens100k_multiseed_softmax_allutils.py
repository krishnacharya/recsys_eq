    ...
    for d in dimension:
        for seed in seeds:
            ...
            for nprod in n_prodarr:
                ...
                dir_prods, prod_utils, user_utils = get_all_engagement_utilities(NE, nue, prob_type='softmax')
                pbar.update(1)
                di = {
                    'dimension': d,
                    'seed': seed,
                    ...
                    'NE': NE_compact,
                    'iters': iters,
                    'user_dist': nue.sum(axis=0) / nue.sum(), # sum embeddings along rows
                    'prod_dist': NE_compact / nprod
                }
                res.append(di)
df = pd.DataFrame(res)
df.to_pickle('../saved_frames/Movielens100k_multiseed_softmax_allutils_30prodmore.pkl')
