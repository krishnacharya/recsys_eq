...
def get_all_exposure_utilities(producers:np.ndarray, user_array:np.ndarray, prob_type='linear', temp = 1): # TODO change
    '''
        ...
        Returns 
            dir_producers direction of basis vector for each producer, shape (N_producers, )
            *exposure* utility for each producer, shape (N_producers, )
            *engagement* utility for each user, shape (N_users, )
    '''
    prodt = producers.T  # resulting shape (dimension, N_prod)
    dir_producers = np.argmax(prodt, axis=0)
    ratings = user_array @ prodt # shape (N_user, N_prod) ratings ij has what user i rates producer j's content <c_i, s_j>
    prob = None
    Nprod = producers.shape[0]
    Nuser = user_array.shape[0]
    if prob_type == 'linear':
        prob = ratings / ratings.sum(axis=1)[:, None] # prob_ij stores <c_i, s_j> / sum_k <c_i, s_k>, shape (N_user, N_prod)
    elif prob_type == 'random':
        prob = np.full((Nuser, Nprod), 1.0 / Nprod)
    elif prob_type == 'softmax':
        exp_ratings = np.exp(ratings / temp)
        prob = exp_ratings / exp_ratings.sum(axis=1)[:, None] # prob_ij stores exp(<c_i, s_j>) / sum_k exp(<c_i, s_k>), prob that user i goes to producer j
    else:
        raise NotImplementedError
...
class ProducersExpsoureGame: # TODO change here too for prob object!
    ...
    def get_best_response(self, current_vec: np.ndarray, remaining_array:np.ndarray) -> np.ndarray:
        '''
            Best response for a producer, when all the other producers are the remaining_array
            Parameters:
                current_vec is a numpy array shape (dimension, )
                remaining_array is a numpy array of shape (N_producers - 1, dimension)
            Returns
                best_row is a numpy array of shape (d,), 
                doing a vertex search which is sufficient when the utility is convex
        '''
        max_util = exposure_utility(self.probability_function(current_vec, remaining_array, self.users.user_array, temp = self.temp))
        best_row = current_vec
        for row in np.eye(self.dimension):
            probs = self.probability_function(row, remaining_array, self.users.user_array, temp=self.temp)
            util = exposure_utility(probs)
            if util > max_util:
                best_row = row
                max_util = util
        return best_row
    ...
    def find_update_best_response(self, producers: np.ndarray):
        '''
            ...
            Note: We search amongst indices randomly
        '''
        for i in np.random.permutation(self.num_producers):
            br = self.get_best_response(producers[i], producers[(np.arange(self.num_producers) != i), :]) # picks rows other than i for remaining_array
            if not np.all(producers[i] == br): # found a best response, update and return
                producers[i] = br
                return producers, False
    ...
    def best_response_dynamics(self, max_iter = 500, verbose = False):
        ...
        for i in range(max_iter):
            producers, converged = self.find_update_best_response(producers)
            if verbose: # for diagnozing
                print(f'##### PRODUCERS FOR ITER {i} \n {producers.sum(axis=0)}')
                _, prod_exp_util, user_eng_util = get_all_exposure_utilities(producers, self.users.user_array, prob_type=self.prob_str, temp = self.temp)
                tot_utilarr.append(prod_exp_util.sum())
                print('Total utility', tot_utilarr[-1])
                print('Producer utilities', prod_exp_util)
