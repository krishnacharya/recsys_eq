...
#SBATCH --output=./Sbatch-exp/sparse-pdutil.%A\_%a.out
#SBATCH --error=./Sbatch-exp/sparse-pdutil.%A\_%a.error
...
python main_run_pdutils_exposuregame.py --data ${data} --prob ${prob} --temp ${temp} --common_config ${cc} --emb_seed ${embseed} --spfrac ${spfrac}